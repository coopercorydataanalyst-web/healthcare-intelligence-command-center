...
hospitals = {
    "GulfStar Medical Center": (310, 1.25),
    "GulfStar North": (190, 0.78),
    "GulfStar Community": (135, 0.55),
}
units = ["Emergency", "Medical-Surgical", "ICU", "Cardiology", "Orthopedics", "Women & Infants"]
dates = pd.date_range("2025-01-01", "2026-08-07", freq="D")
daily = []
for hospital, (beds, scale) in hospitals.items():
    for d in dates:
        ...
        boarding = max(0.2, rng.normal(7.0 * census / beds, 1.4))
        ...
        overtime = max(0, staff_hours * rng.normal(.075 + max(0, census / beds - .86), .018))
        ...
        cost = revenue * rng.normal(.91, .035) + overtime * 38
        denials = revenue * np.clip(rng.normal(.047, .007), .025, .075)
        daily.append([d, hospital, beds, admissions, ed, discharges, census, boarding, lwbs, readm,
                      mortality, falls, hai, staff_hours, overtime, or_cases, or_util, revenue, cost,
                      denials, max(1, rng.normal(15 - 3 * scale, 2.5)), rng.poisson(.16)])

cols = ["date","hospital","staffed_beds","admissions","ed_arrivals","discharges","census",
        "boarding_hours","lwbs_rate","readmission_rate","mortality_rate","falls","hai",
        "staff_hours","overtime_hours","or_cases","or_utilization","revenue","cost","denials",
        "specialty_wait_days","privacy_events"]
...
for i in range(18000):
    ...
    base_cost = {"Heart Failure":17000,"Sepsis":29000,"COPD":13500,"Diabetes":11000,
                 "Joint Replacement":24000,"Maternal Care":14500}[diagnosis]
    cost = max(2500, rng.normal(base_cost * (1 + .16*(los-3)), base_cost*.18))
    revenue = cost * rng.normal(1.08, .12)
    enc.append([f"SYN-{i+1:06d}",hospital,admit,admit+pd.Timedelta(days=los),diagnosis,
...
ecols=["encounter_id","hospital","admit_date","discharge_date","service_line","age","sex","race_ethnicity",
       "payer","svi_quartile","discharge_barrier","followup_booked","readmission_30d","deterioration","harm","los","cost","revenue"]
...
priv=[]
for i in range(420):
    d=rng.choice(dates); h=rng.choice(list(hospitals)); typ=rng.choice(privacy_types,p=[.38,.23,.18,.08,.13])
    priv.append([d,h,typ,int(rng.integers(1,350)),rng.choice(["Low","Moderate","High"],p=[.55,.34,.11]),
