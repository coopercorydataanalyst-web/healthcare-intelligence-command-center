...
130. Replaced the in-panel clarification dropdown with a temporary modal dialog that appears only for genuinely ambiguous wording and closes after selection. Added entity-aware month parsing for arbitrary named lists and inclusive ranges (for example, February and August; January, February, and August; or January through June and December), and preserved those named entities in any clarification choice; advanced the build/state contract to `2026.08.20-v21-entity-aware-dialog`.
131. Added a dedicated Executive KPI Cards performance-intent layer. “Best,” “lowest/weakest,” “critical,” and “how can I improve” questions now rank operating KPIs on their disclosed direction-aware 0–100 threshold scores instead of falling through to trend summaries; critical questions explicitly state when no KPI crosses the modeled critical threshold, and improvement answers provide KPI-specific owners, validation checks, and action pathways; advanced the build/state contract to `2026.08.20-v22-kpi-performance-intents`.
132. Converted the floating visual assistant into a continuous Q&A workflow. The natural-language question field now remains editable after every answer, an Ask button submits new or follow-up questions and replaces the displayed answer, and a Clear button removes both the question and answer through a safe widget callback. Clarification selections are returned to the same editable field after the temporary popup closes; advanced the build/state contract to `2026.08.20-v23-continuous-visual-qa`.
133. Corrected six reviewer-reproduced Q&A failures: modeled priority exposure now returns the true maximum instead of zero; worst readmission and longest specialty-wait hospital questions route correctly; top executive priority wording is recognized; displayed mortality ties name every tied hospital; and unrelated language such as “chair rail” can no longer trigger HAI.
134. Preserved the literal `None` discharge-barrier category during encounter ingestion with `keep_default_na=False`, restoring all 18,000 encounters to Sheets 5 and 14 instead of dropping 9,979 rows during grouped analyses.
135. Added a real offline census-forecasting pipeline in `ml/`: leakage-safe lag features, Ridge and gradient-boosting candidates, last-value and seven-day seasonal baselines, six rolling-origin 30-day folds, split-conformal uncertainty, committed artifacts, and a model card.
