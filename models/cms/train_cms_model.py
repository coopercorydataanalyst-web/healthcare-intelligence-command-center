...
def cross_validated_predictions(frame, features, target, pipeline, splitter, groups=None):
    """Return nested-calibrated OOF predictions; only the supplied splitter differs."""
    rows = []
    split_args = (features, target, groups) if groups is not None else (features, target)
    for fold, (train_index, test_index) in enumerate(splitter.split(*split_args), 1):
        ...
        if groups is not None:
            ...
            inner_splits = inner.split(x_train, y_train, g_train)
        else:
            inner = StratifiedKFold(n_splits=4, shuffle=True, random_state=42 + fold)
            inner_splits = inner.split(x_train, y_train)
        ...
        for position, raw, calibrated in zip(test_index, raw_probability, calibrated_probability):
            rows.append({
                ...
                "fold_hospitals": int(len(test_index)), "fold_positive_rate": float(y_test.mean()),
            })
...
def main():
    ...
    predictions = cross_validated_predictions(frame, x, y, base, GroupKFold(n_splits=5), groups)
    joined = frame.reset_index(drop=True).join(predictions.set_index("row"), how="left")
    random_predictions = cross_validated_predictions(
        frame, x, y, base, StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    )
    ...
    metrics = {
        ...
        "random_split_auc": float(roc_auc_score(random_predictions.actual, random_predictions.raw_probability)),
        "random_split_brier": float(brier_score_loss(random_predictions.actual, random_predictions.raw_probability)),
        "random_split_auc_95_interval": bootstrap_auc_interval(
            random_predictions.actual.to_numpy(), random_predictions.raw_probability.to_numpy()
        ),
        "validation_comparison": "Both protocols use identical features, preprocessing, estimator, five OOF folds, and scoring; only the splitter changes.",
